...
import { ALDEN_SYSTEM_PROMPT } from '../ai/prompts';
import { CLAUDE_API_KEY } from '../apiKey';

const SUGGESTIONS = [
  'How do you migrate our Oracle data?',
  'Where does our data live after migration?',
  'How does the AESO integration work?',
  "What's the parallel run approach?",
];
...
export default function AldenPanel({ isOpen, onClose, onToggle, apiKey }) {
  ...
  const sendMessage = async (text) => {
    ...
    if (cached) {
      ...
      try {
        ...
        let fullText = '';
        await streamClaude({
          systemPrompt: ALDEN_SYSTEM_PROMPT,
        ...
        const errMsg = err?.message?.includes('401')
          ? 'Invalid API key. Check VITE_CLAUDE_API_KEY in your .env file.'
      ...
      setMessages((m) => [
        ...
        {
          role: 'assistant',
          content: 'AI features require a configured API key. Set VITE_CLAUDE_API_KEY in your .env file to enable live responses.',
          error: true,
        },
      ]);
    }
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.data?.type !== 'demo-scroll-read' || e.data?.panel !== 'alden') return;
      if (scrollTimerRef.current) {
        clearInterval(scrollTimerRef.current);
        scrollTimerRef.current = null;
      }
      const container = document.querySelector('[data-demo="alden-messages"]');
      if (!container) return;
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'Syne, sans-serif' }}>
            Your System Companion
          </div>
        </div>
        <button
          type="button"
          data-demo="alden-close"
          onClick={onClose}
          className="p-1 text-lg text-[#C8C4BF] hover:text-[#F2F0EC]"
        >
          ×
        </button>
      </div>
      ...
      <div
        ...
        style={{ display: 'flex', flexDirection: 'column', gap: 10, background: '#161714' }}
        data-demo="alden-messages"
      >
        {messages.map((m, i) => (
          <div
            key={i}
        ...
        {loading && (
          <div className="flex gap-1" style={{ color: '#C8C4BF' }}>
      ...
      >
        ...
        <div className="flex gap-2">
          <input
            type="text"
            data-demo="alden-input"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && sendMessage()}
            placeholder="Ask Alden..."
